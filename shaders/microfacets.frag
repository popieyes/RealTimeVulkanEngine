...
float linearDepth( float depth )
{
    ...
    return ( 2.0f * per_frame_data.m_clipping_planes.x * per_frame_data.m_clipping_planes.y ) 
    / (per_frame_data.m_clipping_planes.y + per_frame_data.m_clipping_planes.x  - z * (per_frame_data.m_clipping_planes.y - per_frame_data.m_clipping_planes.x )); 
}
...
void main() {
    ...
    out_material        = vec4( 1.0, per_object_data.objects[f_instance].m_metallic_roughness.g, per_object_data.objects[f_instance].m_metallic_roughness.r, 1.0 ); //0 for diffuse
}